...
void effCalc(const  Int_t nParticle = 2, // 0-2 : xi, 3-5 : omega
            const TString fileMC = "/Users/rnepeiv/workLund/PhD_work/run3omega/cascadeAnalysisSQM/data/25apr_lhc24b1/AnalysisResults.root",
            const TString fileMCPP = "/Users/rnepeiv/workLund/PhD_work/run3omega/postprocessingo2physics/results/run3_13tev/xi/lhc24b1/25apr/AnalysisResults.root",
            const TString fileCalibration = "/Users/rnepeiv/workLund/PhD_work/run3omega/cascadeAnalysisSQM/efficiencies/effCalibration_LHC24b1.root",
            ...
            const TString postFix = "_LHC24b1") // postfix to the output file
{
  ...
  for (Int_t i = 1; i <= hDummyEffAccInClasses->GetNbinsX(); i++)
    hDummyEffAccInClasses->SetBinContent(i, 1e-12);
  canvasEffAccInClasses->cd();
  padEffAccInClassesUp->cd();
  StyleHistoMultiPlot(hDummyEffAccInClasses, 1e-6, 1e2, 1, 1, "#it{p}_{T} (GeV/#it{c})", "eff. x. acc.", "", 0, 0, 0, 1.5, 1.0, 0, 0.0, 0.05, 0.0, 0.035, 0.005);
  ...
  StyleHistoMultiPlot(hDummyEffSignalInClasses, 0.4, 1.05, 1, 1, "#it{p}_{T} (GeV/#it{c})", "Signal loss", "", 0, 0, 0, 1.5, 1.0, 0, 0.0, 0.05, 0.0, 0.035, 0.005);
  ...
  LegendTitleEventCorr->AddEntry("", "LHC24b1, INEL > 0", "");
  ...
  hRecCollisions1D = (TH1D*)hRecCollisions1D->Rebin(numMultEff, "hRecCollisions1DRebinned", NChFT0MCalibReversedEff);
  ...
  hGenCollisions1D = (TH1D*)hGenCollisions1D->Rebin(numMultEff, "hGenCollisions1DRebinned", NChFT0MCalibReversedEff);
  ...
  Double_t multiplicityPercEventFacotr[numMultEff + 1 + 1] = {0}; 
  for (int i = 0; i < numMultEff + 1; ++i) {
      multiplicityPercEventFacotr[i] = multiplicityPercEff[i];
  }
  multiplicityPercEventFacotr[numMultEff + 1] = 150;

  TH1D* eventCorr = new TH1D("eventCorr", "eventCorr", numMultEff + 1, multiplicityPercEventFacotr);
  std::cout << "\n************* Calibration Values For Efficiencies *************\n";
  long int totalRec = 0;
  long int totalGen = 0;
  for (int i = 1; i <= eventCorr->GetNbinsX(); ++i) {
      double k = hRecCollisions1D->GetBinContent(numMultEff + 1 - i);
      double n = hGenCollisions1D->GetBinContent(numMultEff + 1 - i);
      totalRec += k;
      totalGen += n;
